...
Raw client logs and temp databases were kept under `<run-root>` during the run
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `curl` | 8.7.1 | Passed: `OPTIONS` 204, `/.well-known/caldav` 301, `PROPFIND` root/principal/home/calendar 207 | Passed: `GET` 200 and `HEAD` 200 for seeded object | Passed: `PUT` `VEVENT` 201, `PUT` `VTODO` 201 | Passed: repeat `PUT` returned 204 for event and task | Passed: `DELETE` 204, deleted object `GET` 404 | Passed | Passed: broad `REPORT` 207 included events/tasks; time-ranged `REPORT` 207 included the timed event and omitted the all-day event outside the range | Compatible with the prototype surface |
| `vdirsyncer` | 0.20.0 on Python 3.14.4 | Passed: discovered `/caldav/`, principal, calendar home, and `Compat` collection | Passed: initial sync downloaded events and tasks to a local vdir | Passed: synced a khal-created `VEVENT` and todoman-created `VTODO` to the adapter with `PUT` 201 | Failed/requires follow-up: todoman completion update was resolved as a conflict and copied remote state back over the local change | Passed: todoman delete synced through vdirsyncer with `DELETE` 204 and the object disappeared from server reports | Passed for list/create/delete; update incomplete | Not covered beyond server-side `curl` time-range | Partially compatible; follow-up `op-jty` tracks sync semantics |
...
| `todoman` | 4.7.0 on Python 3.14.4 | Via vdirsyncer | Passed: listed `Seed task` and `Client task updated` from the synced vdir | Passed with explicit `--priority medium`; vdirsyncer uploaded `Todoman client task` and server `REPORT` included it | Failed/requires follow-up: marking the task done locally did not reach the server because vdirsyncer resolved the item as a conflict | Passed: deleting the task synced to the server with `DELETE` 204 | Passed for list/create/delete; update incomplete | Not applicable | Partially compatible through vdirsyncer |
...
- vdirsyncer uses `calendar-multiget` `REPORT` after collection listing. The
  adapter currently parses that as a broad report and returns all objects rather
  than honoring the requested `href` set. Initial sync still succeeded, but this
  is not correct CalDAV behavior.
- vdirsyncer/todoman update sync is unreliable. After todoman marked a task
  complete, vdirsyncer reported conflict resolution for that item and copied the
  remote object back to the local vdir; the server report did not contain
  `STATUS:COMPLETED` for the task.
...
Follow-up `op-jty` tracks the server-side sync semantics needed before
vdirsyncer-style clients should be considered compatible.
...
iCalendar import/export path, but practical client support needs at least
proper `calendar-multiget`, stable sync/update behavior, and a deliberate
auth/TLS/account setup story.
